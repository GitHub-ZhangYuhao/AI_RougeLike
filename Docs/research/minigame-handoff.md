...
> **复核注记（2026-08-22）**：上表「微信全量档 ≤30MB ✅」的结论不成立。用当前 AppID 在微信开发者工具实测预览 preset.2（`wx/`，总包 ~29MB），必现 `subpackage __FULL__ ... exceed max limit 4096KB`。根因：账号未开通「虚拟支付」能力时，微信小游戏主包+分包总量上限是 **20MB**（开通虚拟支付才是 30MB），与是否配置分包无关——当前导出结构已是单一 `engine/` 分包，仍受总量约束。**现行发布方案改为统一用 preset.3（slim 档，`runnable` 已改 `true`）作为微信正式发布档**，总包 19.2–19.4MB，在 20MB 上限内，已过微信开发者工具预览。若要继续用全量档，需二选一：a) 业务侧在 mp.weixin.qq.com 开通虚拟支付（需资质审核）；b) 把全量档资源进一步压到 20MB 内。
