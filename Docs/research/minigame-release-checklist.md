...
> **复核注记（2026-08-22）**：上面「微信小游戏总包 ≤30MB」和「微信端全量档即可直接发布」的结论**不成立，已被真机预览证伪**。用当前 AppID（`wxe982a8234ada9560`）在微信开发者工具实测 preset.2 全量档（`wx/`，总包 29.08MB）预览，必现 `subpackage __FULL__ ... exceed max limit 4096KB`。查证微信官方文档与社区案例后确认：**30MB 上限只在账号已开通「虚拟支付」能力时生效；未开通虚拟支付的账号，主包+所有分包总量上限是 20MB**，与包体是否配置分包无关（本项目导出结构已是单一 `engine/` 分包，仍受总量约束——分包只解决「首屏下载量」，不解决「总量上限」）。抖音 slim 档（19.2–19.4MB）在两种口径下都达标，已过微信开发者工具预览验证。**现行发布方案**：微信也统一用 preset.3 slim 档发布（`export_presets.cfg` preset.3 `runnable` 已改 `true`）；若要维持全量档画质，需业务侧在 mp.weixin.qq.com 后台开通虚拟支付（需资质审核）或继续压缩全量档到 20MB 内，二者均未执行。
