...
| 4 | 撤离收益 | 临时背包物品入仓库 + 「当前波数 × 1.5」暗晶结算奖励（四舍五入） |
...
[新增] menu ──开始──▶ opening ──▶ playing ──▶ (Boss波清空) extraction
        ▲                                        │撤离            │继续
        │                                        ▼                ▼
        └──────── summary ◀────────── (波数×1.5暗晶+背包入库)      playing(休整→下一波)
        ▲
...
| `extraction` | 撤离抉择（新增）：展示本次战利品清单、波数×1.5 奖励预览、「撤离 / 继续深入」按钮；战斗暂停 |
...
| 文件 | 改动 |
|------|------|
| `js/config.js` | 新增 `CONFIG.meta`：掉落表、材料卖价、属性价格曲线、波次奖励系数（×1.5） |
...
2. 撤离流程：Boss 波清空 → `extraction` 状态；选撤离 → 背包入仓库 + 暗晶 = round(波数×1.5) → `summary` → 存档已写入
