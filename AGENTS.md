...
原型侧（`js/`）：2 空格缩进、分号、单引号字符串，import 显式带 `.js` 扩展名。类名 PascalCase（如 `WaveDirector`），函数和变量 camelCase（如 `createEnemyByType`），文件名 kebab-case（如 `rare-items.js`）。保持渲染、状态更新和配置三类职责分离。只在机制或约束不明显时才添加注释。
