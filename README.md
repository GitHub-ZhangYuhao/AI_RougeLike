...
- **离散波次系统**：普通波从 12 只起每波 +5（上限 80），清场后休整 6 秒；第 8 波起出现半血狂化追击怪，第 11 波完全替换基础追击怪；远程兵同屏 ≤4 且每波 ≤12%；每 3 波保证精英，每 5 波进入 Boss 波
...
js/
  ...
  systems/waves.js    离散波次、休整、精英波与 Boss 波
...
- [ ] Step 4（部分完成）：离散波次与 Boss 已完成；宝箱系统待做
