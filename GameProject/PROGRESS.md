...
3. **最终回归与导出检查**：两端小游戏打包均达标——微信全量档总包 29.05MB ≤ 30MB、抖音 slim 档 19.37MB ≤ 20MB（slim 档收紧 env 长边 512→384 补齐差额，分支 perf/minigame-etc2）；剩余：全量 verify / 键鼠 UI 流程复核、两端真机验证与提审准备。
...
| 2026-08-20 | M6 | 小游戏包体达标与打包工具链：全纹理切 ETC2（ctex 132.27→19.92MB，-85%）+ BGM 重编码（音频 2.41→1.52MB）+ 双预设统一 78 条死资产排除；preset.3 由「排除全部美术目录」重设计为 slim 导入尺寸档（整目录排除破坏 preload()，废弃）；新增 minigame_export.ps1（自动剥离/恢复 MCPRuntime + slim 档 .import 快照舞）/ minigame_size_limit.gd（default/slim 两档）/ exclude_check 与 unused_scan 工具；实测全量档总包 29.05MB（微信 30MB 达标）/ slim 档 20.69MB（抖音 20MB 微超 0.7MB），两档 pck 运行时验证零错误，双 smoke 459 项 / 26 场景全绿 | 8df843a / 34e6a00 |
| 2026-08-20 | M6 | 抖音小游戏包体达标：slim 档收紧 env 长边 512→384（12 张环境贴图 512²→384²），slim 总包 20.69→19.37MB、pck 14.31→13.00MB（ctex 11.55→10.24MB），微信/抖音两端均无需 CDN；UI 与角色贴图未动，slim pck 运行时验证零脚本错误，双 smoke 459 项 / 26 场景全绿，发布清单同步复核注记 | 8a86e46 / 45a6886 |
| 2026-08-21 | M6 | 小游戏发布交接落档：新增 Docs/research/minigame-handoff.md 交接快照（状态快照/续作 TODO 8 项/常用命令/环境注记），Docs/README.md 地图登记并将发布清单状态改为定稿 v2，GameProject/AGENTS.md 追加打包踩坑 4 条；无代码与资产改动，双档包体维持达标（微信全量 29.05MB / 抖音 slim 19.37MB） | （本次提交） |
