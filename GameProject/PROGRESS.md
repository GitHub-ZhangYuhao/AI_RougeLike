...
1. **M6 调试 / 性能 / 打磨**：完整 UI、Debug Runtime 与全部 23 章回归已完成；继续推进对象池和 140 敌人性能检查。
...
- [x] 完整 UI：HUD、卡牌、菜单、商城、仓库、撤离、死亡、结算与 Boss/任务提示正式接入
- [x] `logic/debug_runtime.gd`（跨 reset 存活 + serialize）
- [x] `scenes/game/debug_overlay.gd`（F2，替代原型 DOM 面板）
