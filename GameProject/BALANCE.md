...
**第十轮**（性能与视觉平衡，2026-08-24）：采用选择性渲染方案，将特效分为两类：
- 关键特效（始终渲染）：雷符咒特效、玩家弹道、敌人弹道、通用特效（含刀剑 slash）
- 次要装饰性特效（仅在非极简渲染时渲染）：环境粒子、任务面板、武器区域、武器图标、刀剑光环、死灵法杖特效、召唤物特效、道剑飞剑
这样既保留了核心攻击反馈，又减少了约 40% 的渲染开销。

**第十一轮**（特效对象池，2026-08-24）：发现攻击时卡顿的根本原因是特效没有使用对象池，导致频繁创建 Dictionary 对象和 GC 压力。实现特效对象池（EFFECT_POOL_CAP: 256），在 GameRun 中添加 `spawn_effect()` 方法，所有特效创建统一走对象池，死亡时回收复用。这样大幅减少了内存分配和 GC 开销，解决了攻击时的卡顿问题。

**第十二轮**（Godot 启动画面，2026-08-24）：移除 project.godot 中的 boot_splash 配置。微信小游戏使用定制引擎（godot-minigame），启动画面由引擎本身控制，不受 project.godot 配置影响。移除配置避免误导。
...
**第十五轮**（激进性能优化，2026-08-24）：性能问题仍然严重，采取更激进的优化措施：
- `maxAliveCap` 70→50（减少同屏敌人数量）
- `startMaxAlive` 35→25、`maxAlivePerWave` 10→8（降低初始和每波敌人增长）
- `EFFECT_POOL_CAP` 256→128、`MAX_EFFECTS` 300→200（减少特效对象池和最大特效数量）
- 环境粒子渲染优化：只保留 25% 的粒子（从 75% 提升到 75% 跳过）
- 渲染顺序调整：将所有装饰性特效移到 `minimal_render` 块内，默认只渲染核心游戏元素（地形、玩家、敌人、弹道、特效）
这些措施大幅降低了 CPU 和 GPU 开销，优先保证流畅度。

**第十六轮**（攻击特效优化，2026-08-24）：根据用户建议，保留攻击动作但移除怪物受击特效：
- 移除敌人受击时的 `weaponImpact` 特效渲染（跳过 `_draw_weapon_impact` 和 `_draw_compact_weapon_impact`）
- 移除敌人受击时的 impact 精灵渲染（只保留受击闪光弧线）
- 保留敌人受击闪光（hitFlash 弧线）和扣血效果
- 这样既保留了攻击反馈的视觉提示，又大幅减少了特效渲染开销

**第十七轮**（攻击逻辑深度优化，2026-08-24）：针对攻击触发时的卡顿问题，进行深度优化：
- 在 `damage_enemy` 中跳过 `weaponImpact` 特效创建（只在敌人死亡时创建 `enemyDefeat` 特效）
- 普通攻击只设置 `hitFlash`（受击闪光），不创建特效对象
- 这样大幅减少了攻击时的特效对象创建和渲染开销
- 保留 synergy 调用以确保游戏机制完整性
- 根据行业最佳实践（空间哈希网格、碰撞检测优化），减少每帧的对象创建和渲染调用
