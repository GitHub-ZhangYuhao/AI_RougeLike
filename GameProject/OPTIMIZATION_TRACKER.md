...
## 第四轮：6 武器 Lv6 质变视觉差异化（设计完成，待实现）

### 现状分析

| 武器 | Lv6 机制 | 视觉差异化 | 评价 |
|------|---------|-----------|------|
| 道剑 | 剑意飞剑 (10把, 链式6跳) | 专用 _draw_flying_swords()，独立精灵图、轨道→突刺→返回状态机 | ? 强 |
| 披风 | 炽热冲击 (3s CD, 击杀强化) | 专用 flipbook cloak_fire_burst_anim 25帧 + 亮度阿尔法重塑形 | ? 强 |
| 雷符 | 引雷 (首目标瞬雷+AOE+3跳链) | 复用 bolt_fx/chain_fx，与低等级无差异 | ? 弱 |
| 法杖 | 百鬼夜行 (击杀招魂+持续治疗) | 复用 _draw_staff_effects()，与低等级无差异 | ? 弱 |
| 玉戒 | 终极 (狂暴+反击新星) | 仅轨道颜色变化+简单扩散圆 | ?? 中弱 |
| 丹炉 | 九转热烟 (永久热区) | 复用 furnace 渲染，与低等级无差异 | ? 弱 |

### 设计方案

#### 1. 雷符 Lv6「天雷引」— 雷域 + 天罚

- **永久雷域光环**：玩家周围 6 道弧形电弧缓慢旋转（半径~180px），draw_arc + 正弦抖动模拟电场
- **天罚落雷升级**：thunderFirst 触发时从屏幕顶部画粗闪电柱（6→10px），落点 3 层同心圆扩散雷环 + 0.08s 全屏白色 vignette
- **AOE 雷爆**：thunderAoE 命中时地面短暂焦黑圆斑（0.5s 淡出）+ 8 道辐射分叉闪电
- **链式闪电升级**：chain_fx 更粗（3→5px），颜色淡黄→纯白蓝，命中点十字闪光

实现成本：纯 _draw_talisman_effects() 扩展，不需新精灵图。

#### 2. 法杖 Lv6「百鬼夜行」— 鬼火队列 + 灵魂收割

- **夜行模式视觉**：nightParade 激活时召唤物 tint 偏紫蓝 + 紫色外发光光晕 + 拖尾粒子
- **灵魂收割特效**：击杀触发 CONVERT_CHANCE 时，紫色贝塞尔曲线灵魂丝线从敌人飞向新生召唤物（0.3s）
- **夜行治疗光环**：heal_player 激活时玩家脚下旋转紫色法阵（2 层反向圆弧 + 4 符文点）
- **鬼火队列**：召唤物>3 时松散队列跟随，每个身后 3 帧位置历史残影

实现成本：_draw_summons() 加 nightParade 分支 + _draw_staff_effects() 增加法阵/灵魂线。可能需 ghost_minion.png 或纯 tint。

#### 3. 玉戒 Lv6「极意」— 玉卫阵列 + 冰裂反击

- **狂暴变身**：frenzy 时 6 环膨胀加速，轨道变双圈交错（内外各 3），颜色翠绿→赤金，火焰尾迹
- **反击新星升级**：counter_nova 先 0.15s 冻结帧 → 12 道冰裂纹辐射（白色+分叉）→ 淡蓝扩散消散；被冻敌人画冰晶碎片
- **血玉膨胀升级**：bloodDrop expand 到最大时释放压力波（细线圆环快速外扩），收缩时环向内吸入
- **常驻气场**：相邻玉环间半透明能量连线

实现成本：_draw_weapon_zones() ring 分支扩展 + counter_nova 升级。纯代码渲染。

#### 4. 丹炉 Lv6「九转」— 丹成异象 + 热域升华

- **丹炉升格**：furnace 外圈八角形炉壁（深红线条），内部火焰橙→金红，顶部 3 缕金色烟气（正弦曲线粒子）
- **九转热区**：hotZone 从不可见→半透明金色热浪扭曲区域（多层 draw_circle），边缘热气上升粒子
- **丹成异象**（核心质变标志）：furnace 存活>5s 时炉顶出现旋转金色光环（修仙"丹成"天象），逐渐扩大；消亡时光环收缩爆散为 8 道金色射线
- **热区链接**：2 个 hotZone 距离<200px 时画金色热流连线

实现成本：_draw_trails() furnace/hotZone 渲染扩展。纯代码渲染。
...
|--------|------|------|
| P0 | 雷符 | 最容易实现，视觉冲击最大 |
| P1 | 玉戒 | counter_nova + frenzy 变身，战斗反馈提升明显 |
| P2 | 法杖 | nightParade 视觉重塑，工作量中等 |
| P3 | 丹炉 | 丹成异象概念好但渲染细节多，可分步 |
