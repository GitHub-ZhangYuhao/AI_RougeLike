...
func run(runner) -> void:
    ...
    var capped = Factory.create_enemy_by_type("chaser", 0, 0, 0, 20)
    runner.check(is_equal_approx(early.maxHp, 45.0), "[8] wave 1 HP")
    runner.check(is_equal_approx(late.maxHp / early.maxHp, 2.1), "[8] wave 11 HP scaling")
    runner.check(is_equal_approx(early.speed, 102.6) and is_equal_approx(capped.speed, 121.6), "[8] wave speed curve (Rng 0.5 -> jitter 1.0)")
