...
func _draw_hud(size: Vector2) -> void:
	...
	draw_string(ThemeDB.fallback_font, Vector2(26.0, 47.0), "境界  Lv %d" % run.level, HORIZONTAL_ALIGNMENT_LEFT, -1.0, 19, GOLD)
	...
	draw_string(ThemeDB.fallback_font, Vector2(28.0, 91.0), "生命  %d / %d" % [ceili(maxf(0.0, run.player.hp)), ceili(run.player.maxHp)], HORIZONTAL_ALIGNMENT_LEFT, -1.0, 14, BONE)
	_draw_weapon_slots()
	var center_x: float = size.x * 0.5
	_draw_panel(Rect2(center_x - 176.0, 16.0, 352.0, 52.0), Color(0.025, 0.05, 0.043, 0.84), Color(JADE, 0.48))
	draw_string(ThemeDB.fallback_font, Vector2(center_x - 100.0, 38.0), _format_time(run.elapsed), HORIZONTAL_ALIGNMENT_CENTER, 200.0, 23, BONE)
	draw_string(ThemeDB.fallback_font, Vector2(center_x - 170.0, 59.0), _wave_text(), HORIZONTAL_ALIGNMENT_CENTER, 340.0, 14, GOLD if run.waveDirector.isBossWave else MUTED)
	var right_panel := Rect2(size.x - 206.0, 20.0, 194.0, 92.0)
	_draw_panel(right_panel, INK, Color(JADE, 0.6))
	var alive: int = run.enemies.filter(func(enemy) -> bool: return not enemy.dead).size()
	draw_string(ThemeDB.fallback_font, Vector2(size.x - 192.0, 48.0), "斩敌  %d" % run.kills, HORIZONTAL_ALIGNMENT_LEFT, 170.0, 16, BONE)
	draw_string(ThemeDB.fallback_font, Vector2(size.x - 192.0, 72.0), "妖物  %d" % alive, HORIZONTAL_ALIGNMENT_LEFT, 170.0, 15, MUTED)
	draw_string(ThemeDB.fallback_font, Vector2(size.x - 192.0, 95.0), "暗夜领主  %d" % run.bossesDefeated, HORIZONTAL_ALIGNMENT_LEFT, 170.0, 14, Color("c6a6dc"))
	_draw_boss_bar(size)
	_draw_task_panel(size)
	_draw_inventory(size)
	draw_string(ThemeDB.fallback_font, Vector2(18.0, size.y - 17.0), "WASD / 方向键移动   ·   武器自动攻击   ·   点击武器槽选择联动", HORIZONTAL_ALIGNMENT_LEFT, -1.0, 12, Color(MUTED, 0.72))
...
func _draw_weapon_slots() -> void:
	...
	for i in rects.size():
		...
		_draw_panel(rect, Color(0.04, 0.075, 0.06, 0.92), GOLD if selected else Color(JADE, 0.55), 2.0 if selected else 1.0)
		if weapon == null:
			draw_string(ThemeDB.fallback_font, rect.position + Vector2(0.0, 33.0), "空", HORIZONTAL_ALIGNMENT_CENTER, rect.size.x, 13, Color(MUTED, 0.45))
			continue
		var id: String = weapon.card["id"]
		var color: Color = CARD_COLORS.get(id, BONE)
		draw_circle(rect.position + Vector2(rect.size.x * 0.5, 22.0), 13.0, Color(color, 0.2))
		draw_string(ThemeDB.fallback_font, rect.position + Vector2(0.0, 27.0), CARD_SIGILS.get(id, "武"), HORIZONTAL_ALIGNMENT_CENTER, rect.size.x, 15, color)
		draw_string(ThemeDB.fallback_font, rect.position + Vector2(0.0, rect.size.y - 7.0), "Lv%d" % weapon.level, HORIZONTAL_ALIGNMENT_CENTER, rect.size.x, 11, GOLD)
...
func _draw_boss_bar(size: Vector2) -> void:
	...
	_draw_panel(Rect2(rect.position - Vector2(5.0, 20.0), rect.size + Vector2(10.0, 38.0)), Color(0.055, 0.035, 0.07, 0.92), Color("b99b52"))
	...
	draw_string(ThemeDB.fallback_font, Vector2(rect.position.x, rect.position.y - 5.0), boss.name if not boss.name.is_empty() else "暗夜领主", HORIZONTAL_ALIGNMENT_CENTER, rect.size.x, 15, Color("ead69c"))
...
func _draw_task_panel(size: Vector2) -> void:
	...
	draw_string(ThemeDB.fallback_font, rect.position + Vector2(14.0, 25.0), "奇遇 · %s  T%d" % [type_names.get(task["type"], task["type"]), task["tier"]], HORIZONTAL_ALIGNMENT_LEFT, rect.size.x - 28.0, 16, Color("74d5d0"))
	draw_string(ThemeDB.fallback_font, rect.position + Vector2(14.0, 48.0), state_names.get(task["state"], task["state"]), HORIZONTAL_ALIGNMENT_LEFT, rect.size.x - 28.0, 13, MUTED)
...
func _draw_inventory(size: Vector2) -> void:
	...
	for id: String in run.rareInventory:
		...
		if count > 0:
			lines.append("%s ×%d" % [_rare_name(id), count])
	if not lines.is_empty():
		var height: float = 30.0 + lines.size() * 18.0
		var rect := Rect2(12.0, 164.0, 190.0, height)
		_draw_panel(rect, Color(0.055, 0.07, 0.055, 0.88), Color(GOLD, 0.55))
		draw_string(ThemeDB.fallback_font, rect.position + Vector2(12.0, 22.0), "稀有物品", HORIZONTAL_ALIGNMENT_LEFT, 160.0, 14, GOLD)
		for i in lines.size():
			draw_string(ThemeDB.fallback_font, rect.position + Vector2(12.0, 43.0 + i * 18.0), lines[i], HORIZONTAL_ALIGNMENT_LEFT, 165.0, 12, BONE)
	...
	if not backpack.is_empty():
		draw_string(ThemeDB.fallback_font, Vector2(size.x - 360.0, size.y - 18.0), "临时行囊  " + "  ".join(backpack), HORIZONTAL_ALIGNMENT_RIGHT, 342.0, 13, Color("c7a6df"))
...
func _draw_choice(size: Vector2) -> void:
	...
	draw_string(ThemeDB.fallback_font, Vector2(0.0, top_y - 48.0), title, HORIZONTAL_ALIGNMENT_CENTER, size.x, 29, Color("f0dfaf"))
	draw_string(ThemeDB.fallback_font, Vector2(0.0, top_y - 20.0), "点击卡牌或按数字键 1-%d" % offers.size(), HORIZONTAL_ALIGNMENT_CENTER, size.x, 13, MUTED)
	...
	for i in offers.size():
		...
		_draw_panel(rect, Color(0.035, 0.075, 0.06, 0.97) if hover else Color(0.025, 0.055, 0.045, 0.96), GOLD if hover else Color(accent, 0.72), 3.0 if hover else 1.5)
		...
		draw_string(ThemeDB.fallback_font, rect.position + Vector2(10.0, 30.0), str(i + 1), HORIZONTAL_ALIGNMENT_CENTER, 28.0, 14, BONE)
		draw_string(ThemeDB.fallback_font, rect.position + Vector2(42.0, 28.0), TYPE_LABELS.get(offer.get("type", ""), "奖励"), HORIZONTAL_ALIGNMENT_RIGHT, rect.size.x - 54.0, 12, accent)
		var sigil_center := rect.position + Vector2(rect.size.x * 0.5, rect.size.y * 0.32)
		draw_circle(sigil_center, minf(38.0, rect.size.y * 0.14), Color(accent, 0.17))
		draw_arc(sigil_center, minf(38.0, rect.size.y * 0.14), 0.0, TAU, 28, Color(accent, 0.75), 2.0)
		draw_string(ThemeDB.fallback_font, sigil_center + Vector2(-35.0, 9.0), CARD_SIGILS.get(id, "赐"), HORIZONTAL_ALIGNMENT_CENTER, 70.0, 27, accent)
		draw_string(ThemeDB.fallback_font, rect.position + Vector2(10.0, rect.size.y * 0.53), card.get("name", "未知奖励"), HORIZONTAL_ALIGNMENT_CENTER, rect.size.x - 20.0, 20, BONE)
		draw_string(ThemeDB.fallback_font, rect.position + Vector2(10.0, rect.size.y * 0.63), _level_info(offer), HORIZONTAL_ALIGNMENT_CENTER, rect.size.x - 20.0, 13, GOLD)
		...
		for line_index in mini(lines.size(), 4):
			draw_string(ThemeDB.fallback_font, rect.position + Vector2(16.0, rect.size.y * 0.74 + line_index * 18.0), lines[line_index], HORIZONTAL_ALIGNMENT_CENTER, rect.size.x - 32.0, 12, Color(BONE, 0.76))
...
func _draw_modal(size: Vector2, title: String, body: String, accent: Color) -> void:
	...
	draw_string(ThemeDB.fallback_font, Vector2(rect.position.x, rect.position.y + 78.0), title, HORIZONTAL_ALIGNMENT_CENTER, rect.size.x, 36, accent)
	var lines: PackedStringArray = body.split("\n")
	for i in lines.size():
		draw_string(ThemeDB.fallback_font, Vector2(rect.position.x + 30.0, rect.position.y + 126.0 + i * 30.0), lines[i], HORIZONTAL_ALIGNMENT_CENTER, rect.size.x - 60.0, 17, BONE)
...
func _draw_key_button(rect: Rect2, key: String, label: String, accent: Color) -> void:
	...
	draw_string(ThemeDB.fallback_font, rect.position + Vector2(10.0, 33.0), "[%s]" % key, HORIZONTAL_ALIGNMENT_CENTER, 58.0, 14, GOLD)
	draw_string(ThemeDB.fallback_font, rect.position + Vector2(64.0, 34.0), label, HORIZONTAL_ALIGNMENT_CENTER, rect.size.x - 74.0, 17, BONE)
...
func _draw_announcements(size: Vector2) -> void:
	if run.state == "playing" and run.waveDirector.bannerTimer > 0.0:
		...
		draw_string(ThemeDB.fallback_font, Vector2(0.0, size.y * 0.23), label, HORIZONTAL_ALIGNMENT_CENTER, size.x, 31, Color(GOLD, alpha))
	...
	draw_string(ThemeDB.fallback_font, Vector2(0.0, size.y * 0.28), text, HORIZONTAL_ALIGNMENT_CENTER, size.x, 24, color)
	if not detail.is_empty():
		draw_string(ThemeDB.fallback_font, Vector2(0.0, size.y * 0.28 + 25.0), detail, HORIZONTAL_ALIGNMENT_CENTER, size.x, 14, BONE)
...
func _draw_card_corners(rect: Rect2, color: Color) -> void:
	...
	for corner in [rect.position, rect.position + Vector2(rect.size.x, 0.0), rect.position + rect.size, rect.position + Vector2(0.0, rect.size.y)]:
		var sx: float = 1.0 if corner.x == rect.position.x else -1.0
		var sy: float = 1.0 if corner.y == rect.position.y else -1.0
		draw_line(corner, corner + Vector2(length * sx, 0.0), color, 2.0)
		draw_line(corner, corner + Vector2(0.0, length * sy), color, 2.0)
