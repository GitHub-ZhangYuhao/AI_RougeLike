...
export function applyWaveScaling(enemy, wave = 1) {
  ...
  // Waves 1-6 ramp faster, 7-11 keep pressure, and 12+ flatten toward the HP cap.
  ...
  const speedCapStep = Math.max(1, CONFIG.enemy.speedCapStartWave - 1);
  const speedProgress = Math.min(step, speedCapStep) / speedCapStep;
  const speedMult = CONFIG.enemy.baseSpeedMult
    + (CONFIG.enemy.speedWaveCap - CONFIG.enemy.baseSpeedMult) * speedProgress;
